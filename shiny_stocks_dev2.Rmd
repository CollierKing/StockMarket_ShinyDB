...
#### Current RSI Quantile

```{r-RSI quantile gauge}
...
renderGauge({
  ...
  gauge(rsi_q, min = 0, max = 1, symbol = '%', gaugeSectors(
  success = c(.8, 1), warning = c(.4, .79), danger = c(0, .39)
...
event_chart <- reactive({
    ...
    if(substr(event,nchar(event)-1,nchar(event))=="dn"){
        ...
        g1 <- ggplot(data = event_summ_tbl, aes(x = event_summ_tbl[,1], y = return,fill=rsi_q)) +
          geom_bar(stat = "identity",alpha = 0.7) + ggtitle("Min_Return") +
            coord_flip() +  scale_x_discrete(limits = rev(levels(event_summ_tbl[,1]))) +  
            scale_fill_manual(values = c("[0,0.2)" = "tomato1", "[0.2,0.4)" = "orange", "[0.4,0.6)" = "gold2", "[0.6,0.8)" = "lightgreen", "[0.8,1]" = "springgreen4")) + xlab("") +ylab("") + theme(legend.position="none")
            
        
        g2 <- ggplot(data = event_summ_tbl, aes(x = event_summ_tbl[,1], y = min_day, fill=rsi_q)) +
            geom_bar(stat = "identity",alpha = 0.7) + ggtitle("Min_Day") +
         coord_flip() +  scale_x_discrete(limits = rev(levels(event_summ_tbl[,1]))) +    
            scale_fill_manual(values = c("[0,0.2)" = "tomato1", "[0.2,0.4)" = "orange", "[0.4,0.6)" = "gold2", "[0.6,0.8)" = "lightgreen", "[0.8,1]" = "springgreen4")) + xlab("") +ylab("") + theme(legend.position="none")
        ...
        g2 <- ggplot(data = event_summ_tbl, aes(x = event_summ_tbl[,1], y = max_day, fill=rsi_q)) +
            geom_bar(stat = "identity",alpha = 0.7) + ggtitle("Max_Days") + coord_flip() + scale_x_discrete(limits = rev(levels(event_summ_tbl[,1]))) + scale_fill_manual(values = c("[0,0.2)" = "tomato1", "[0.2,0.4)" = "orange", "[0.4,0.6)" = "gold2", "[0.6,0.8)" = "lightgreen", "[0.8,1]" = "springgreen4")) + xlab("") +ylab("") + theme(legend.position="none")
...
selectInput("reporting_per", label = "Choose Fin. Statement Type: ",
             list(`Annual` = "annual",
                `Quarterly` = "quarter"), selected = "Quarterly")
...
finplot <- reactive({
    ...
    #WIP
    #Edit -
    #do this in separate plot and then grid arrange them, then dont need to worry about rescaling a second axis?
    # if table row is selected, add line
    
    if(!is.null(input$tbl_is_rows_selected)){
        row <- tbl_is_rows_selected
    } else
    if(!is.null(input$tbl_bs_rows_selected)){
        row <- tbl_bs_rows_selected
    } else
    if(!is.null(input$tbl_cf_rows_selected)){
        row <- tbl_cf_rows_selected
    } else 
    if(!is.null(input$tbl_ratios_valuation_rows_selected)){
        row <- tbl_ratios_valuation_rows_selected
    } else
    if(!is.null(input$tbl_ratios_financial_rows_selected)){
        row <- tbl_ratios_financial_rows_selected
    } else
    if(!is.null(input$tbl_ratios_profitability_rows_selected)){
        row <- tbl_ratios_profitability_rows_selected
    } else
    if(!is.null(input$tbl_ratios_growth_rows_selected)){
        row <- tbl_ratios_growth_rows_selected
    } else
    if(!is.null(input$tbl_ratios_cashflow_rows_selected)){
        row <- tbl_ratios_cashflow_rows_selected
    } else
    if(!is.null(input$tbl_ratios_financial_health_rows_selected)){
        row <- tbl_ratios_financial_health_rows_selected
    } else
    if(!is.null(input$tbl_ratios_efficiency_rows_selected)){
        row <- tbl_ratios_efficiency_rows_selected
    }
    
    
    # test2 <- test %>% gather(date,val,-category) #debugging
    test2 <- selectedtbl() %>% gather(date,val,-category)
    
    # test2 <- test2[row,]$category
    
    # cat <- selected_tbl()[input$selected_tbl_rows_selected,]$category
    cat <- test2[row,]$category #debugging
    
    test2 <- test2 %>% filter(
        category == cat
    )
    # test2 <- test2[test2$category=="Revenue",] #debugging
...
# 
# p2 <- ggplot() + geom_point(data=input,aes(x=date,y=cross_val,color="red"),size=5) + xlab('') + ylab('') + theme(legend.position='none',axis.title.x = element_blank()) + scale_x_bd(business.dates=input$date, max.major.breaks=10, labels=date_format("%b '%y"))  + ylab("") + xlab("") + theme(axis.title.x = element_blank()) + background_grid(major = "xy",minor = "none") + scale_y_continuous(limits=c(0,max(input$cross_val))) + ggtitle(paste0(test2$category))
...
dataTableOutput("selectedtbl")

output$selectedtbl <- renderDataTable({
    selectedtbl()
},
options = list(
    scrollY = '300px', paging = FALSE, scrollX = TRUE),selection='single'
...
renderText(
    lol()
    # input$selectedtbl_rows_selected
)
