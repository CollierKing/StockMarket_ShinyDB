...
finplot <- eventReactive(input$clickMe_fins,{
    ...
    test2 <- test %>% gather(date,val,-category)
    test2 <- test2[test2$category=="Revenue",]
...
p2 <- ggplot(input) + geom_col(data = input, aes(date, cross_val/6, fill = "myfill"))

    # p1 <- p1 + scale_y_continuous(sec.axis = sec_axis(~.*5, name = "Revenue"))
    # 
    # p1 + geom_col(data = input, aes(date, cross_val/6, fill = "myfill"))
...
renderText(
    ...
    input$tbl_is_rows_selected
)

proxy = dataTableProxy('tbl_is')
