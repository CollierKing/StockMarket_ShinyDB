...
## Add checkboxes
# BUYING ACTIVITY
checkboxInput("buying_activity", "Option Buying Activity",value = TRUE)
                        
# SELLING ACTIVITY
checkboxInput("selling_activity", "Option Selling Activity",value = TRUE)
...
textInput("inText","Ticker Search:","")

# actionButton("goButton",
#               "Search", class = "btn btn-lg btn-primary", 
#              onclick="location.href=#Second Section;")

actionButton("clickMe", label = "Search", class = "btn btn-lg btn-primary", 
...
selectedData <- reactive({
    ...
    #Create top bullish trade table
    g_file_sub %>% 
        group_by(Ticker) %>%
        summarise(
...
selectedData2 <- reactive({
    ...
    ticker <- g_file_sub_all[input$tbl1_rows_selected,]$Ticker
    g_file_sub <- g_file_sub %>% arrange(Ticker,desc(TradeDate))
...
event_chart <- reactive({
    ...
    if(substr(event,nchar(event)-1,nchar(event))=="dn"){
        ...
        geom_bar(stat='identity',width=.5) + xlab(paste0("Occurance count: ",event)) +
            geom_ribbon(aes(ymin = return_mean - return_sd, ymax = return_mean + return_sd), fill = "grey70",alpha=0.3) + geom_line(aes(y = return_mean),colour="red") + ggtitle("Farthest Return Distribution")
        
        g2 <- ggplot(event_summ_tbl, aes(x=event_summ_tbl[,1], y=min_day)) +
        geom_bar(stat='identity',width=.5) + xlab(paste0("Occurance count: ",event)) +
            geom_ribbon(aes(ymin = day_mean - day_sd, ymax = day_mean + day_sd), fill = "grey70",alpha=0.3)  + geom_line(aes(y = day_mean),colour="red") + ggtitle("Farthest Day Distribution")
...
event_summ_tbl <- eventReactive(input$clickMe4,{
    ...
    for(k in 1:nrow(event_summary)){
        ...
        if(substr(event,nchar(event)-1,nchar(event))=="dn"){
            ...
            summarise(min_price = min(adjusted),
                      ...
                      min_day = sum(which(adjusted == min_price)),
                      tot_days = n()) %>%
            summarise(avg_min_return = mean(return,na.rm=TRUE),
            ...
            summarise(max_price = max(adjusted),
                      ...
                      max_day = sum(which(adjusted == max_price)),
                      tot_days = n()) %>%
            summarise(avg_max_return = mean(return,na.rm=TRUE),
        ...
        event_summary$total_occurances[k] = max(input2[,event])
    }
...
Options Activity
    ...
    * Add BUYING/SELLING checkbox logic
