...
event_chart <- reactive({
    ...
    if(substr(event,nchar(event)-1,nchar(event))=="dn"){
        ...
        g2 <- ggplot(data = event_summ_tbl, aes(x = event_summ_tbl[,1], y = min_day, fill=rsi_q)) +
         ...
         coord_flip() +  scale_x_discrete(limits = rev(levels(event_summ_tbl[,1]))) +            scale_fill_manual(values= c("tomato1","orange", "gold2", "lightgreen","springgreen4","grey")) + xlab("") +ylab("") + theme(legend.position="right")
        ...
        g2 <- ggplot(data = event_summ_tbl, aes(x = event_summ_tbl[,1], y = max_day, fill=rsi_q)) +
            geom_bar(stat = "identity",alpha = 0.7) + ggtitle("Max_Days") +
         coord_flip() +  scale_x_discrete(limits = rev(levels(event_summ_tbl[,1]))) +            scale_fill_manual(values= c("tomato1","orange", "gold2", "lightgreen","springgreen4","grey")) + xlab("") +ylab("") + theme(legend.position="right")
    ...
    matches <- which(input2$date %in% event_summ_tbl$date)
        ...
        if(length(matches)>0){
            ...
            for (i in 1:length(matches)) {
                loop_input <- paste("geom_vline(xintercept=input2$date[",matches[i],
...
Options Activity
    ...
    * remove 'date' axis title from chart
...
Technicals - Events
    ...
    * Color code bars by RSI quintile
        - Added/Fix with legend colors to be constant
