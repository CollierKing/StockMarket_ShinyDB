...
selectedData2 <- reactive({
   ...
   # validate(
   #    need(!is.na(input$inText), "Please select a data set")
   #  )
    
        #View Activity via table selection
    if (nchar(input$inText)<1){
        
        # validate(need(input$tbl1_rows_selected, message = FALSE))
        # validate(need(try(!is.na(input$tbl1_rows_selected), "Please select a data set")))
        # validate(
        #     need(try(input$tbl1_rows_selected != ""), "Please select a data set")
        # )
        ...
        # validate(need(input$inText, message = FALSE))
       # validate(need(try(!is.na(input$inText), "Please select a data set")))
        # validate(
        #     need(try(input$inText != ""), "Please select a data set")
        # )
...
event_chart <- reactive({
    ...
    if(substr(event,nchar(event)-1,nchar(event))=="dn"){
        ...
        #     geom_ribbon(aes(ymin = return_mean - return_sd, ymax = return_mean + return_sd,group=1), fill = "grey70",alpha=0.3) + geom_line(aes(y = return_mean),colour="red",group=1) + ggtitle("Farthest Return Distribution") + scale_x_discrete(limits = rev(levels(event_summ_tbl[,1])))
        # 
        # g2 <- ggplot(event_summ_tbl, aes(x=event_summ_tbl[,1], y=min_day)) +
        # geom_bar(stat='identity',width=.5) + xlab(paste0("Occurance count: ",event)) +
        #     geom_ribbon(aes(ymin = day_mean - day_sd, ymax = day_mean + day_sd,group=1), fill = "grey70",alpha=0.3)  + geom_line(aes(y = day_mean),colour="red",group=1) + ggtitle("Farthest Day Distribution") + scale_x_discrete(limits = rev(levels(event_summ_tbl[,1])))
        ...
        #TEST
        # ggplot(test_tbl, aes(x=ID,y=value,fill=factor(key))) +
        # geom_bar(stat="identity",position="dodge") + scale_x_discrete(limits = test_tbl$ID)
...
#             geom_ribbon(aes(ymin = return_mean - return_sd, ymax = return_mean + return_sd,group=1), fill = "grey70",alpha=0.3) + geom_line(aes(y = return_mean),colour="red",group=1) + ggtitle("Farthest Return Distribution") + scale_x_discrete(limits = rev(levels(event_summ_tbl[,1])))
#             
#         g2 <- ggplot(event_summ_tbl, aes(x=event_summ_tbl[,1], y=max_day)) +
#         geom_bar(stat='identity',width=.5) + xlab(paste0("Occurance count: ",event)) +
#             geom_ribbon(aes(ymin = day_mean - day_sd, ymax = day_mean + day_sd,group=1), fill = "grey70",alpha=0.3)  + geom_line(aes(y = day_mean),colour="red",group=1) + ggtitle("Farthest Day Distribution") + scale_x_discrete(limits = rev(levels(event_summ_tbl[,1])))
...
event_summ_all <- eventReactive(input$clickMe4,{
    ...
    #check for weekly checkbox
    if (input$time_frame2=="Daily"){
...
Options Activity
    ...
    * Add chartlinks to sidebar
    * Add financial link to sidebar
    * Add earnings link to sidebar
    * Add inst link to sidebar
...
Technicals - Events
    ...
    * Add legend for mean/sd line/interval on chart
        - Added
    * Color code bars by RSI quintile
        - Added
    * Change color coded bars to better color pallete
    * Add events to chart
        * add first date to each event
        * add horizontol line + event count to chart
...
Fundamentals
    ...
    * switch source to yhoo or google
